...
#include "../../linux-2.6.36.x/net/nat/hw_nat/ra_nat.h"
#include "../../linux-2.6.36.x/net/nat/hw_nat/frame_engine.h"
#endif
...
/* Modify by chz for 2.6.36 kernel, 2013-06-06 */
...
static int __init pptp_init_module(void)
{
	...
	if (gre_add_protocol(&gre_pptp_protocol, GREPROTO_PPTP) < 0) {
		printk(KERN_INFO "PPTP: can't add protocol GREPROTO_PPTP\n");
		goto out_free_mem;
	}
#elif LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0)
	...
	if (inet_add_protocol(&net_pptp_protocol, IPPROTO_GRE) < 0) {
		printk(KERN_INFO "PPTP: can't add protocol IPPROTO_GRE\n");
		goto out_free_mem;
	}
#endif
