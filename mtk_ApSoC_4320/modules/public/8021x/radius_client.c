...
#define RADIUS_INIT_RETRY_INTERVAL   3
...
{		
    ...
    do {
    	...
    	if (ret == 0 && Radius_change_server(rtapd, rtapd->conf->mbss_auth_server[apidx], NULL, rtapd->radius->mbss_auth_serv_sock[apidx], 1))
    		ret = -1;

    	if (ret == 0 && eloop_register_read_sock(rtapd->radius->mbss_auth_serv_sock[apidx], Radius_client_receive, rtapd, (void *) RADIUS_AUTH))
    	{
    	...
    	{
            if (rtapd->radius->mbss_auth_serv_sock[apidx] >= 0)
                close(rtapd->radius->mbss_auth_serv_sock[apidx]);
            DBGPRINT(RT_DEBUG_ERROR,"%s error. %dth retry %dseconds later\n", __FUNCTION__, ++retry, RADIUS_INIT_RETRY_INTERVAL);
            sleep(RADIUS_INIT_RETRY_INTERVAL);
    	}
    } while (ret != 0 && !eloop_terminated());
	return ret;
}
#else
static int Radius_client_init_auth(rtapd *rtapd)
{
    ...
    do {
        ...
        if (ret == 0 && Radius_change_server(rtapd, rtapd->conf->auth_server, NULL, rtapd->radius->auth_serv_sock, 1))
    		ret = -1;

    	if (ret == 0 && eloop_register_read_sock(rtapd->radius->auth_serv_sock, Radius_client_receive, rtapd, (void *) RADIUS_AUTH))
    	{
    	...
    	if (ret == -1)
    	{
            ...
            DBGPRINT(RT_DEBUG_ERROR, "%s error. %dth retry %d seconds later\n", __FUNCTION__, ++retry, RADIUS_INIT_RETRY_INTERVAL);
            sleep(RADIUS_INIT_RETRY_INTERVAL);
    	}
    } while (ret != 0 && !eloop_terminated());
