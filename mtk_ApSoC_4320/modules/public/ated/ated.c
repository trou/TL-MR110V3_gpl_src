...
static void RaCfg_Agent(void)
{
	...
	while (sighup_flag) 
	{
		...
		else
		{
			...
			{                
				memcpy(&rcv_protocol, buffer+12 +4/*have 802.1q*/, 2);
...
#if 0
/* TODO: why check it here? by xiejiabai@tp-link.com.cn */
	...
	perror("my_eth_addr(ifr.ifr_hwaddr.sa_data)");
	printf("my_eth_addr(ifr.ifr_hwaddr.sa_data):%02x:%02x:%02x:%02x:%02x:%02x\n",
            my_eth_addr[0],my_eth_addr[1],my_eth_addr[2],my_eth_addr[3],my_eth_addr[4],my_eth_addr[5]);
	return 0;
...
static void NetReceive(u8 *inpkt, int len)
{
	...
	p_racfgh = (struct racfghdr *) &inpkt[ETH_HLEN + 4/*have 802.1q*/];
        /* get the vlan tag of the input packet*/
        memcpy(vlan_tag, &inpkt[12], 4);
#ifdef DBG
    u32 index = 0;
    printf("eth packet including vlan tag: \n");
    for (index=0;index<ETH_HLEN + 4;index++)
        printf("0x%x ", inpkt[index]);
	...
	if (/*(p_ehead->h_proto == htons(ETH_P_RACFG)) no need this && */
	...
	if (Command_Id == RACFG_CMD_AP_START)
	{
		...
		memcpy(&packet[14 + 4/*has 802.1q*/], p_racfgh, Len + 12 + sizeof(pid));
		pwrq.u.data.length = Len + 12 + sizeof(pid);
	}
	else
	{
		memcpy(&packet[14 + 4/*has 802.1q*/], p_racfgh, Len + 12);
		pwrq.u.data.length = Len + 12;
	}
	pwrq.u.data.pointer = (caddr_t) &packet[14 + 4/*has 802.1q*/];
	strncpy(pwrq.ifr_name, IF_NAME, IFNAMSIZ);
	...
	p_racfgh = (struct racfghdr *)&packet[14 + 4/*has 802.1q*/];
    ...
    /*802.1q header*/
	/* use vlan tag of the input packet, by xiejiabai@tp-link.com.cn */
#if 0
    packet[12] = 0x81;
    packet[13] = 0x00;
    packet[14] = 0x00;
