...
struct timer_list ralink_wdt_timer;
#define RALINK_WDT_TIMER_FREQ (HZ * 3)

#define WATCHDOG_TIMEOUT 20		/* 10 sec default timeout */
...
static void ralink_wdt_do_timer(unsigned long unused)
{
    /* feed the dog */
    //printk("ralink_wdt_do_timer feed dog.\n");
    RaWdgReload();

	init_timer(&ralink_wdt_timer);
	ralink_wdt_timer.expires = jiffies + RALINK_WDT_TIMER_FREQ;
	add_timer(&ralink_wdt_timer);
}

static void ralink_wdt_timer_init(void)
{
    /* start watchdog */
    RaWdgStart();

	init_timer(&ralink_wdt_timer);
	ralink_wdt_timer.function = ralink_wdt_do_timer;
	ralink_wdt_timer.expires = jiffies + RALINK_WDT_TIMER_FREQ;
	add_timer(&ralink_wdt_timer);
}
