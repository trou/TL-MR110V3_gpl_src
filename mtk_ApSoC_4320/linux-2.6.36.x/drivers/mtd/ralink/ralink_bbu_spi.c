...
#if defined(CONFIG_TP_MODEL_MR100V1) //MR100V1
#define TP_BOOT_SIZE 		0x10000
#define TP_KERNEL_SIZE 		0x110000
#define TP_ROOTFS_SIZE		0x2C0000
#define TP_CONFIG_SIZE		0x10000
#define TP_ROMFILE_SIZE		0x0
#define TP_ROM_SIZE			0x0 //no use
#define TP_RADIO_SIZE		0x10000

#elif defined (CONFIG_TP_MODEL_MR100V2) //MR100V2
#define TP_BOOT_SIZE 		0x20000
#define TP_KERNEL_SIZE 		0x130000
#define TP_ROOTFS_SIZE		0x560000
#define TP_CONFIG_SIZE		0x10000
#define TP_CONFIGBAK_SIZE   0x10000
#define TP_ISPCONFIG_SIZE	0x10000
#define TP_ROMFILE_SIZE		0x10000
#define TP_ROM_SIZE			0x0 //no use
#define TP_RADIO_SIZE		0x10000
#define TP_USERDATA_SIZE	0x100000
#else

#define TP_BOOT_SIZE 		0x20000
#if defined(CONFIG_TP_MODEL_MR3420V5) || defined (CONFIG_TP_MODEL_MR3020V3)
#define TP_KERNEL_SIZE 		0x150000
#define TP_ROOTFS_SIZE		0x650000
#elif defined (CONFIG_TP_MODEL_MR6400V3) || defined (CONFIG_TP_MODEL_MR6400V5) || defined (CONFIG_TP_MODEL_MR200V5)
#define TP_KERNEL_SIZE 		0x130000
#define TP_ROOTFS_SIZE		0x680000
#elif defined (CONFIG_TP_MODEL_MR402V1)
#define TP_KERNEL_SIZE 		0x140000
#define TP_ROOTFS_SIZE		0xB50000
#define TP_USERDATA_SIZE	0x300000
#define TP_CONFIGBAK_SIZE   0x10000
#define TP_ISPCONFIG_SIZE	0x10000
#elif defined (CONFIG_TP_MODEL_MR400V4) && defined (CONFIG_TP_10M_FLASH)
#define TP_KERNEL_SIZE 		0x140000
#define TP_ROOTFS_SIZE		0x870000
#elif defined (CONFIG_TP_MODEL_MR400V4)
#define TP_KERNEL_SIZE 		0x00200000
#define TP_ROOTFS_SIZE		0x00DA0000
#else
#define TP_KERNEL_SIZE 		0x140000
#define TP_ROOTFS_SIZE		0x660000
#endif
#define TP_CONFIG_SIZE		0x10000
#define TP_ROMFILE_SIZE		0x10000
#define TP_ROM_SIZE			0x10000
#define TP_RADIO_SIZE		0x10000
#endif

#if defined (CONFIG_TP_10M_FLASH)
#define TP_CONFIG_OFFSET	(TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE)
#elif defined (CONFIG_TP_MODEL_MR400V4) || defined (CONFIG_TP_MODEL_MR600V2)
#define TP_CONFIG_OFFSET	(TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE + TP_ROMFILE_SIZE)
#elif defined (CONFIG_TP_MODEL_MR100V2) || defined (CONFIG_TP_MODEL_MR402V1)
#define TP_CONFIG_OFFSET	(TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE + TP_ISPCONFIG_SIZE)
#define TP_CONFIGBAK_OFFSET	(TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE + TP_ISPCONFIG_SIZE+TP_CONFIG_SIZE)
#else
#define TP_CONFIG_OFFSET	(TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE)
#endif
...
#ifndef CONF_FLASH_SIGNATURE
#define CONF_FLASH_SIGNATURE			0x98765432
#endif

int flash_version = 1;
EXPORT_SYMBOL(flash_version);

#ifdef TP_FLASH_IOCTL
/* added by yangxv for flash ioctl, 2009.03.06 */
#include <linux/fs.h>
...
static u_char l_buf[FLASH_MAX_RW_SIZE] = {0};
static struct semaphore	l_buf_lock;
...
static struct mtd_partition rt2880_partitions[] = {
    {
        name:		"boot",
	 size:		TP_BOOT_SIZE,/* 0x000000000000-0x000000020000  */
	 offset:		0,
    },
    {
        name:		"kernel",
	 size:		TP_KERNEL_SIZE,/* 0x000000020000-0x000000160000  */
	 offset:		TP_BOOT_SIZE,
    },
    {
        name:		"rootfs",
	 size:		TP_ROOTFS_SIZE,/* 0x000000160000-0x0000007c0000  */
	 offset:		TP_BOOT_SIZE + TP_KERNEL_SIZE,
    },
    {
        name:		"config",
	 size:		TP_CONFIG_SIZE, /* 0x0000007c0000-0x0000007d0000  */
	 offset:		TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE,
    },
    {
        name:		"romfile",
	 size:		TP_ROMFILE_SIZE,/* 0x0000007d0000-0x0000007e0000  */
	 offset:		TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE +TP_CONFIG_SIZE,
    },
    {
        name:		"rom",
	 size:		TP_ROM_SIZE,/* 0x0000007e0000-0x0000007f0000  */
	 offset:		TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE +TP_CONFIG_SIZE + TP_ROMFILE_SIZE,
    },
    {
        name:		"radio",
	 size:		TP_RADIO_SIZE,/* 0x0000007f0000-0x000000800000  */
	 offset:		TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE +TP_CONFIG_SIZE + TP_ROMFILE_SIZE + TP_ROM_SIZE,
    }
    /* add end  */
...
static struct mtd_partition rt2880_partitions[] = {
...
#if defined(CONFIG_TP_MODEL_MR100V2) || defined (CONFIG_TP_MODEL_MR402V1)
    {
        name:		"ispconfig",
	 size:		TP_ISPCONFIG_SIZE, /* 0x0000007c0000-0x0000007d0000  */
	 offset:		TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE,
    },
    {
        name:		"config",
	 size:		TP_CONFIG_SIZE, /* 0x0000007c0000-0x0000007d0000  */
	 offset:		TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE+TP_ISPCONFIG_SIZE,
    },
    {
        name:		"configbak",
	 size:		TP_CONFIG_SIZE, /* 0x0000007c0000-0x0000007d0000  */
	 offset:		TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE+TP_ISPCONFIG_SIZE+TP_CONFIG_SIZE,
    },
   {
        name:		"romfile",
	 size:		TP_ROMFILE_SIZE,/* 0x0000007d0000-0x0000007e0000  */
	 offset:		TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE +TP_ISPCONFIG_SIZE+TP_CONFIG_SIZE+TP_CONFIGBAK_SIZE,
    },
#else
   {
        name:		"config",
   ...
   },
   {
...
#endif
#if defined(CONFIG_TP_MODEL_MR6400V3) || defined (CONFIG_TP_MODEL_MR400V4) || defined (CONFIG_TP_MODEL_MR6400V5) || defined (CONFIG_TP_MODEL_MR200V5) || defined(CONFIG_TP_MODEL_MR100V1)
    {
        name:		"radio",
	 size:		TP_RADIO_SIZE,/* 0x0000007f0000-0x000000800000  */
	 offset:		TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE +TP_CONFIG_SIZE + TP_ROMFILE_SIZE,
    ...
    {
        name:		"radio",
	 size:		TP_RADIO_SIZE,/* 0x0000006F0000-0x000000700000  */
	 offset:		TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE +TP_ISPCONFIG_SIZE+TP_CONFIG_SIZE + TP_CONFIGBAK_SIZE+TP_ROMFILE_SIZE,
    } ,
    {
        name:		"userdata",
	 size:		TP_USERDATA_SIZE,/* 0x000000700000-0x000000800000  */
	 offset:		TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE +TP_ISPCONFIG_SIZE+TP_CONFIG_SIZE +TP_CONFIGBAK_SIZE+ TP_ROMFILE_SIZE+TP_RADIO_SIZE,
    }
#else
    {
        name:		"rom",
...
static struct chip_info chips_data [] = {
	...
	{ "EN25QX64A",          0x1c, 0x71170000, 64 * 1024, 128, 0 },
	{ "EN25QH64A",          0x1c, 0x70171c30, 64 * 1024, 128, 0 },//add by lxy for adapt, 25Nov20
	{ "EN25QH128A",         0x1c, 0x70181c30, 64 * 1024, 256, 0 },//add by lxy for adapt, 25Nov20
	{ "EN25SX128A",         0x1c, 0x78180000, 64 * 1024, 256, 0 },
	{ "EN25QX128A",         0x1c, 0x71180000, 64 * 1024, 256, 0 },
	{ "EN25F32",            0x1c, 0x31161c31, 64 * 1024, 64,  0 },
	{ "EN25F64",            0x1c, 0x20171c20, 64 * 1024, 128, 0 },  // EN25P64
	{ "EN25Q64",            0x1c, 0x30171c30, 64 * 1024, 128, 0 },
	{ "W25Q32BV",           0xef, 0x40160000, 64 * 1024, 64,  0 },//W25Q32FV,W25Q32JV
	{ "W25X32VS",           0xef, 0x30160000, 64 * 1024, 64,  0 },
	{ "W25Q64BV",           0xef, 0x40170000, 64 * 1024, 128, 0 }, //S25FL064K //W25Q64FV ,W25Q64JV
	{ "W25Q128JV",          0xef, 0x40180000, 64 * 1024, 256, 0 },//W25Q128FV,W25Q128BV
	{ "W25Q128JW",          0xef, 0x60180000, 64 * 1024, 256, 0 },
	{ "W25Q256FV",          0xef, 0x40190000, 64 * 1024, 512, 1 },
	{ "XM25QH32C",           0x20, 0x40160000, 64 * 1024, 64, 0 },
	{ "XM25QH64C",          0x20, 0x40178364, 64 * 1024, 128, 0 },
	{ "XM25QH64A",          0x20, 0x70170080, 64 * 1024, 128, 0 },//add by lxy for adapt, 20Nov20
	{ "XM25QH128A",         0x20, 0x70180080, 64 * 1024, 256, 0 },//add by lxy for adapt, 25Nov20
	{ "XM25QH128C",         0x20, 0x40180000, 64 * 1024, 256, 0 },
	...
	{ "GD25Q32B",           0xC8, 0x40160000, 64 * 1024, 64,  0 },//GD25Q32B ,GD25Q32E
	{ "GD25Q64E",           0xC8, 0x40170000, 64 * 1024, 128, 0 },
	{ "GD25Q128E",          0xC8, 0x40180000, 64 * 1024, 256, 0 },//GD25Q128C
	{ "GD25LQ128E",         0xC8, 0x60180000, 64 * 1024, 256, 0 },
...
#ifdef TP_FLASH_IOCTL
	init_MUTEX(&l_buf_lock);
#endif
#endif

	if (chip->id == 0xc8 && chip->jedec_id == 0x40170000) {
		flash_version = 0;
	}

	printk("%s(%02x %04x flash_version = %d) (%d Kbytes)\n",
	       chip->name, chip->id, chip->jedec_id, flash_version, (int)(flash->mtd.size / 1024));
...
#if 0/* remove by wanghao, we do not need this.  */
...
/* Add support to tp bia os, yuanshang, 2013-11-01 
 */
#ifdef TP_FLASH_IOCTL
...
int spiflash_ioctl_read(u_char *buf, u32 from, u32 len)
{
	u32 retlen = 0;
	int ret = 0;
	ret = ramtd_read(&(flash->mtd), from, len, &retlen, buf);
	printk("%s, Read from 0x%08x length 0x%x, ret %d, retlen 0x%x\n",
			__FUNCTION__,
			from,
			len,
			ret,
...
/* 
 * ramtd_write(struct mtd_info *mtd, loff_t to, size_t len,
 *	 size_t *retlen, const u_char *buf)
 */
...
int spiflash_ioctl_write(u_char *buf, u32 to, u32 len)
{
	u32 retlen = 0;
	int ret = 0;
	ret = ramtd_write(&(flash->mtd), to, len, &retlen, buf);
	printk("%s, Write to 0x%08x length 0x%x, ret %d, retlen 0x%x\n",
			__FUNCTION__,
			to,
			len,
			ret,
...
/*
 * ramtd_erase(struct mtd_info *mtd, struct erase_info *instr)
 */
int spiflash_ioctl_erase(u32 to, u32 len)
{
	...
	for (count = 0; count < (len / flash->mtd.erasesize + 1); count++ )
	{
		if ( (len%flash->mtd.erasesize == 0) && (count == len / flash->mtd.erasesize))
		{
			break;
		}	
		ramtd_erase(&(flash->mtd), &instr);
		instr.addr += flash->mtd.erasesize;
		printk(".");
	}
	printk("\n%s, erase to 0x%08x length 0x%x, nerase done\n",
			__FUNCTION__,
			instr.addr,
			len);	
}
...
{
	uint64_t config_offset = TP_CONFIG_OFFSET;
#if defined (CONFIG_TP_MODEL_MR100V2)|| defined (CONFIG_TP_MODEL_MR402V1)
	uint64_t configbak_offset = TP_CONFIGBAK_OFFSET;
#endif

	down(&l_buf_lock);
	if(spiflash_ioctl_read(l_buf, config_offset, FLASH_MAX_RW_SIZE) < 0)
	{
		up(&l_buf_lock);
		return -1;
	}

//	memset(l_buf, '0', 0x100);
	memset(l_buf, '0', 20);

	reset_write = 1;

	if(spiflash_ioctl_write(l_buf, config_offset, 0x10000) < 0)
	{
		memset(l_buf, '0', FLASH_MAX_RW_SIZE);
		reset_write = 0;
		up(&l_buf_lock);
		return -1;
	}
#if defined (CONFIG_TP_MODEL_MR100V2)|| defined (CONFIG_TP_MODEL_MR402V1)
	if(spiflash_ioctl_read(l_buf, configbak_offset, FLASH_MAX_RW_SIZE) < 0)
	{
		up(&l_buf_lock);
		reset_write = 0;
		return -1;
	...
	memset(l_buf, '0', 20);
	if(spiflash_ioctl_write(l_buf, configbak_offset, 0x10000) < 0)
	{
		memset(l_buf, '0', FLASH_MAX_RW_SIZE);
		reset_write = 0;
		up(&l_buf_lock);
...
long flash_ioctl (struct file *filp, unsigned int cmd, unsigned long arg)
{
    struct flash_opt *opt=(struct flash_opt *)arg;
	int need_reboot = 0;
	static int isReset = 0;

	/*printk("tc3162 map is %x, %x, %x, %x, name is %s\n\n\n\n\n", 
		tc3162_map.bankwidth, tc3162_map.phys, tc3162_map.size, tc3162_map.virt,
		tc3162_map.name);*/
	...
	if ( NULL == l_buf )
	{
		printk(KERN_WARNING "Failed to kmalloc FLASH_MAX_RW_SIZE\n");
		return -1;
	}
    ...
    case FLASH_IOCTL_READ:
		if(spiflash_ioctl_read(l_buf, (u32)opt->src, opt->bytes) < 0) {
	    	opt->result=OUT_OF_SCOPE;
		}
		copy_to_user((u_char *)opt->dest, l_buf, opt->bytes);
		break;
    ...
    case FLASH_IOCTL_WRITE:
	{
		...
		{
			local_irq_disable();
			disable_irq(18); //�ر�usb hub�жϣ������ӿ�������µ�crash
		}

		if (TP_CONFIG_OFFSET == (u32)opt->dest)
		{
			if (isReset)
			{
				printk("\033[0;1;5;31;43mafter Reset, can NOT write user config anymore\033[0m\n");
				break;
			}

			if (((USRCONF_STRUCT *)(opt->src))->signature != CONF_FLASH_SIGNATURE)
			{
				printk("\033[0;1;5;31;43mreset now, then can NOT write user config anymore\033[0m\n");
				isReset = 1;
			}
		}
		...
		{
			copy_from_user(l_buf, usr_buf, FLASH_MAX_RW_SIZE);
			spiflash_ioctl_write(l_buf, addr, FLASH_MAX_RW_SIZE);
		...
		if (len)
		{
			copy_from_user(l_buf, usr_buf, len);
			spiflash_ioctl_write(l_buf, addr, len);
		}
		...
		if (need_reboot)
		{
			//rsl层的sleep移到kernel这里来，不确定sleep 30秒有什么用，以防万一保留设计，但是更换大板烧录和sleep的顺序，这样大板子卡一起烧录，降低升级期间断电的风险
			printk("sleep 2 before reboot\n");
			usleep(1000*1000*2);
	...
	case FLASH_IOCTL_GET_UID:
		//if(get_uid_cur(l_buf, opt->bytes) < 0) {
	   	//	opt->result = GET_UID_ERROR;
		//}
		//copy_to_user((u_char *)opt->dest, l_buf, opt->bytes);
		break;
	
	case FLASH_IOCTL_GET_UID_LEN:
		//copy_to_user((u_char *)opt->dest, &cur_uid_len, opt->bytes);
		break;
	
	case FLASH_IOCTL_GET_ID:
	{
		u32 flash_id = flash->chip->jedec_id; /*spiflash_read_id();*/
		copy_to_user((u_char *)opt->dest, &flash_id, opt->bytes);
		break;
	}
