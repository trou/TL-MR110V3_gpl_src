...
typedef void (*backtrace_callback) (struct task_struct *task, const struct pt_regs *regs);
static backtrace_callback g_backtrace_cb = NULL;
void register_backtrace(backtrace_callback callback)
{
	g_backtrace_cb = callback;
}
EXPORT_SYMBOL(register_backtrace);

void unregister_backtrace(void)
{
	g_backtrace_cb = NULL;
}
EXPORT_SYMBOL(unregister_backtrace);

static void show_user_backtrace(struct task_struct *task, const struct pt_regs *regs)
{
	if (g_backtrace_cb)
	{
		g_backtrace_cb(task, regs);
	}
}
