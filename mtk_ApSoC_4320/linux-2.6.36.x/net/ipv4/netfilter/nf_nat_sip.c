...
/*Han Jiayan@20130621 add, For some sip msg's udp/tcp port is not same as SIP contact port*/
	...
	newaddr = addr->ip;
	newport = port;
	map_addr_according_expectition(skb, &newaddr, &newport);
				     ...
				     union nf_inet_addr *rtp_addr)
{
	...
	//printk(KERN_ERR "rtp_exp port=%d rtcp_exp port=%d\n", ntohs(rtp_exp->tuple.dst.u.udp.port), ntohs(rtcp_exp->tuple.dst.u.udp.port));
	...
	//printk(KERN_ERR "rtp_exp->tuple.dst.u.udp.port=%d rtp_exp->saved_proto.udp.port=%d\n", ntohs(rtp_exp->tuple.dst.u.udp.port), ntohs(rtp_exp->saved_proto.udp.port));
	return NF_ACCEPT;
