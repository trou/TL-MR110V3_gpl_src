...
/*注意：TUNNEL_NAME_6TO4宏定义需要与oal_tunnel6.h的6to4接口定义保持一致*/
#define TUNNEL_NAME_6TO4		"6to4"
...
tm_fwd_cb_t ipv6_fwd_cb = NULL;
int tm_register_ipv6_fwd_cb(tm_fwd_cb_t cb)
{
   ipv6_fwd_cb = cb;
}
EXPORT_SYMBOL(tm_register_ipv6_fwd_cb);
			     ...
			     unsigned int len, struct sfe_ipv6_ip_hdr *iph, unsigned int ihl, bool flush_on_find)
{
	...
	if (ipv6_fwd_cb && ipv6_fwd_cb(cm->connection->mark, skb)) {
		struct sfe_ipv6_connection *c = cm->connection;
		sfe_ipv6_remove_connection(si, c);
		si->packets_not_forwarded++;
		spin_unlock_bh(&si->lock);
	 ...
	 */
	if (!ipv6_fwd_cb && cm->connection->mark != 0) {
	...
	if(!strncmp(xmit_dev->name,TUNNEL_NAME_6TO4,sizeof(TUNNEL_NAME_6TO4)))
	{
		if (skb_dst(skb) == NULL)
			ip6_route_input(skb);
	}
 ...
 */
int sfe_ipv6_update_rule(struct sfe_connection_create *sic)
{
	struct sfe_ipv6_connection *c;
	struct sfe_ipv6 *si = &__si6;
 ...
 *  sfe_ipv6_ctl_conns()
 */
void sfe_ipv6_ctl_conns(const ctlrule_t *rules)
{
	struct sfe_ipv6 *si = &__si6;
	struct sfe_ipv6_connection *head;
	ctlrule_t *ctl_rules, *ctl_subs;
	int i = 0, ctl_conn = 0, need_break = 0;
	uint32_t  ctl_flag;
	uint16_t   temp_port;
	...
	for(head = si->all_connections_head;head; head = head->all_connections_next)
	{
		...
		for (ctl_rules = rules; ctl_rules; ctl_rules = ctl_rules->next)
		{
			for (ctl_subs = ctl_rules; ctl_subs; ctl_subs = ctl_subs->next_sub)
			{
				...
				switch(ctl_subs->type)
				{
					...
					default:
						need_break = 1;
						printk("sfe_ipv6 error: not support the type %d!\n", ctl_subs->type);
						break;
				}
			...
			if (!ctl_subs)    //satisfy all subrules,so match the rule
			{
				ctl_conn = 1;
				break;
			}
		...
		if (ctl_conn)
		{
			...
			{
				printk("sfe_ipv6 warning: marks are different!\n");
			}
...
EXPORT_SYMBOL(sfe_ipv6_ctl_conns);
