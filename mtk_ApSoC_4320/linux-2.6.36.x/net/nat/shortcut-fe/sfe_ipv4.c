...
struct sfe_ipv4_connection {
	...
	uint32_t set_qos_mark;      /* whether to set the qos mark bits */
};
...
//[zhangguosong start] 2018-10-22
#define SFE_IPV4_CONNECTION_KEEPALIVE_INTERVAL 8 * HZ
//[zhangguosong end]
...
struct sfe_ipv4 {
	...
	unsigned int max_connections;	/* max connections */
...
static struct proc_dir_entry *sfe_ipv4_entry = NULL;
static struct proc_dir_entry *sfe_stat_entry = NULL;
static struct proc_dir_entry *sfe_conn_entry = NULL;

tm_fwd_cb_t ipv4_fwd_cb = NULL;
int tm_register_ipv4_fwd_cb(tm_fwd_cb_t cb)
{
   ipv4_fwd_cb = cb;
}
EXPORT_SYMBOL(tm_register_ipv4_fwd_cb);
					...
					__be32 dest_ip, __be16 dest_port)
{
	 ...
	 */
	if (unlikely(!cm)) {
		DEBUG_TRACE("Don't have this hash: %d %lu: %d %pI4(%u), %pI4(%u)\n", conn_match_idx, dev, protocol, &src_ip, ntohs(src_port), &dest_ip, ntohs(dest_port));
		return cm;
	}
...
static inline void sfe_ipv4_insert_sfe_ipv4_connection_match(struct sfe_ipv4 *si, struct sfe_ipv4_connection_match *cm)
{
	...
	DEBUG_TRACE("Insert cm: %lu: %d %pI4(%u), %pI4(%u)\n", cm->match_dev, cm->match_protocol, &cm->match_src_ip, ntohs(cm->match_src_port),
															  &cm->match_dest_ip, ntohs(cm->match_dest_port));
...
{
	DEBUG_TRACE("Remove connection: %d %pI4(%pI4):%u(%u) %pI4(%pI4):%u(%u) mark=%x\n",
		c->protocol, &c->src_ip, &c->src_ip_xlate, ntohs(c->src_port), ntohs(c->src_port_xlate),
		&c->dest_ip, &c->dest_ip_xlate, ntohs(c->dest_port), ntohs(c->dest_port_xlate), c->mark);
	/*
						  ...
						  uint64_t now_jiffies)
{
	...
	memcpy(sis->rep_dst_mac, reply_cm->xmit_dest_mac, ETH_ALEN);
			     ...
			     unsigned int len, struct sfe_ipv4_ip_hdr *iph, unsigned int ihl, bool flush_on_find)
{
	...
	/* For l2tp & IPSec ALG */
	if  (ntohs(src_port) == 1701 || ntohs(dest_port) == 1701)
	{
		spin_unlock_bh(&si->lock);
		DEBUG_TRACE("ignore l2tp packet\n");
		return 0;
	}
	if (ntohs(src_port) == 500 || ntohs(dest_port) == 500 || ntohs(src_port) == 4500 || ntohs(dest_port) == 4500)
	{
		spin_unlock_bh(&si->lock);
		DEBUG_TRACE("ignore ipsec packet\n");
		return 0;
	}
	...
	if (ipv4_fwd_cb && ipv4_fwd_cb(cm->connection->mark, skb)) {
		struct sfe_ipv4_connection *c = cm->connection;
		sfe_ipv4_remove_sfe_ipv4_connection(si, c);
		si->packets_not_forwarded++;
		spin_unlock_bh(&si->lock);
	...
	//[zhangguosong start] 2018-10-22
	if (time_after(jiffies, cm->keepaliveTime + SFE_IPV4_CONNECTION_KEEPALIVE_INTERVAL))
	{
		//DEBUG_TRACE("Leave a skb to the default Linux stack.\n");
		cm->keepaliveTime = jiffies;
		si->packets_not_forwarded++;
		spin_unlock_bh(&si->lock);
		return 0;
	}
	...
	//[zhangguosong start] 2018-12-18 Try to match item it control list
#if 0
	if (sfe_match_delete_item(skb, IPPROTO_UDP, src_ip, dest_ip, src_port, dest_port))
	{
		DEBUG_TRACE("%s: Match the sfe connection, just return with 0.\n", __FUNCTION__);
		return 0;
	}
#endif
	//[zhangguosong end]
	...
	if (likely(cm->flags & SFE_IPV4_CONNECTION_MATCH_FLAG_WRITE_L2_HDR)) {
		if (unlikely(!(cm->flags & SFE_IPV4_CONNECTION_MATCH_FLAG_WRITE_FAST_ETH_HDR))) {
			...
			{
				//printk("\n need to add space to skb data:%0x  head:%0x \n",skb->data,skb->head);
			...
			if (skb_cloned(skb) || skb_shared(skb)) {
				...
				printk(KERN_EMERG "sfe_ipv4_recv_tcp:  len:%d  head:%p "
			  "data:%p tail:%#lx end:%#lx dev:%s cloned:%d, shared:%d\n",
	       skb->len, skb->head, skb->data,
	       (unsigned long)skb->tail, (unsigned long)skb->end,
	       skb->dev ? skb->dev->name : "<NULL>",skb_cloned(skb) ,skb_shared(skb) );
		   {
			   	int i;
				for (i = 0; i < skb->len; i++)
				{
			...
			struct sfe_ipv4_eth_hdr *eth = (struct sfe_ipv4_eth_hdr *)skb_push(skb, ETH_HLEN);
	 ...
	 * Add the mark to skb->mark instead of assigning the mark value
	 * to avoid clearing the wan2lan bit 0x8000, which is an essential
	 * part to QoS for model mtk.
	 */
	if (!ipv4_fwd_cb && cm->connection->mark != 0) {
		skb->mark |= cm->connection->mark;
		if (!(cm->connection->set_qos_mark & SFE_CM_CTL_MARK_UDP))
			skb->mark &= (~MASK_FOR_IQOSMARK);
			     ...
			     unsigned int len, struct sfe_ipv4_ip_hdr *iph, unsigned int ihl, bool flush_on_find)
{
	...
	if (likely(!(cm->flags & SFE_IPV4_CONNECTION_MATCH_FLAG_NO_SEQ_CHECK))) {
		...
		if (unlikely((int32_t)(seq - (cm->protocol_state.tcp.max_end + 1)) > 0)) {
			...
			/*si->packets_not_forwarded++;
			spin_unlock_bh(&si->lock);*/
			DEBUG_TRACE("seq: %u exceeds %x right edge: %u\n",
				    seq, cm, cm->protocol_state.tcp.max_end + 1);
			/*sfe_ipv4_flush_sfe_ipv4_connection(si, c, SFE_SYNC_REASON_FLUSH);
			return 0;*/
		}
		...
		if (unlikely((int32_t)(sack - (counter_cm->protocol_state.tcp.end + 1)) > 0)) {
			...
			/*si->packets_not_forwarded++;
			spin_unlock_bh(&si->lock);*/
			DEBUG_TRACE("ack: %u exceeds %x right edge: %u\n",
				    sack, counter_cm, counter_cm->protocol_state.tcp.end + 1);
			/*sfe_ipv4_flush_sfe_ipv4_connection(si, c, SFE_SYNC_REASON_FLUSH);
			return 0;*/
		}
		...
		if (unlikely(cm->protocol_state.tcp.max_win < scaled_win)) {
			DEBUG_TRACE("Update %x max_win: %u -> %u\n", cm, cm->protocol_state.tcp.max_win, scaled_win);
		...
		if (likely((int32_t)(end - cm->protocol_state.tcp.end) > 0)) {
			DEBUG_TRACE("Update %x end: %u -> %u\n", cm, cm->protocol_state.tcp.end, end);
			cm->protocol_state.tcp.end = end;
		}

		max_end = sack + scaled_win;
		if (likely((int32_t)(max_end - counter_cm->protocol_state.tcp.max_end) > 0)) {
			DEBUG_TRACE("Update %x max_end: %u -> %u\n", counter_cm, counter_cm->protocol_state.tcp.max_end, max_end);
	...
	//[zhangguosong start] 2018-12-18 Try to match item it control list
#if 0
	if (sfe_match_delete_item(skb, IPPROTO_TCP, src_ip, dest_ip, src_port, dest_port))
	{
		DEBUG_TRACE("%s: Match the sfe connection, just return with 0.\n", __FUNCTION__);
		return 0;
	}
	...
	if (!ipv4_fwd_cb && cm->connection->mark != 0) {
		skb->mark |= cm->connection->mark;
		if (!(cm->connection->set_qos_mark & SFE_CM_CTL_MARK_TCP))
			skb->mark &= (~MASK_FOR_IQOSMARK);
		DEBUG_TRACE("SKB MARK is NON ZERO %x\n", skb->mark);
	}
			      ...
			      unsigned int len, struct sfe_ipv4_ip_hdr *iph, unsigned int ihl)
{
	...
	si->packets_not_forwarded++;
	spin_unlock_bh(&si->lock);
	DEBUG_TRACE("ICMP removes connection\n");
...
/* 
 * sfe_ipv4_update_rule
 * Update matched rule's protocol state.
 * Return 0 if success, -1 if there's no such rule, mainly for 
 * notifying fast-classifier to remove its cached connection.
 */
int sfe_ipv4_update_rule(struct sfe_connection_create *sic)
{
	struct sfe_ipv4_connection *c;
	struct sfe_ipv4 *si = &__si;
...
int sfe_ipv4_create_rule(struct sfe_connection_create *sic)
{
	...
	/* num_connections limits */
	if(si->max_connections != 0 && si->num_connections >= si->max_connections)
	{
		spin_unlock_bh(&si->lock);
		return -ENOMEM;
	}
	...
	original_cm->keepaliveTime = jiffies;
	...
	reply_cm->keepaliveTime = jiffies;
	...
	case IPPROTO_TCP:
		...
		DEBUG_TRACE("Create %x max_win = %u end = %u max_end = %u\nCreate %x max_win = %u end = %u max_end = %u\n", \
					original_cm, original_cm->protocol_state.tcp.max_win, \
					original_cm->protocol_state.tcp.end, \
					original_cm->protocol_state.tcp.max_end, \
					reply_cm, reply_cm->protocol_state.tcp.max_win, \
					reply_cm->protocol_state.tcp.end, \
					reply_cm->protocol_state.tcp.max_end);
		if (sic->flags & SFE_CREATE_FLAG_NO_SEQ_CHECK) {
 ...
 *  sfe_ipv4_ctl_conns()
 */
void sfe_ipv4_ctl_conns(const ctlrule_t *rules)
{
	struct sfe_ipv4 *si = &__si;
	struct sfe_ipv4_connection *head;
	ctlrule_t *ctl_rules, *ctl_subs;
	int i = 0, ctl_conn = 0;
	uint32_t  ctl_flag;
	uint16_t  temp_port;
	...
	for(head = si->all_connections_head;head; head = head->all_connections_next)
	{
		ctl_conn = 0;
		for (ctl_rules = rules; ctl_rules; ctl_rules = ctl_rules->next)
		{
			for (ctl_subs = ctl_rules; ctl_subs; ctl_subs = ctl_subs->next_sub)
			{
				int need_break = 0;
				if (head->protocol == IPPROTO_TCP && !(ctl_subs->flag & SFE_CM_CTL_PROTO_TCP)) break;
				if (head->protocol == IPPROTO_UDP && !(ctl_subs->flag & SFE_CM_CTL_PROTO_UDP)) break;
				if (head->mark & MASK_FOR_WAN2LAN && !(ctl_subs->flag & SFE_CM_CTL_DIR_W2L))   break;
				if (!(head->mark & MASK_FOR_WAN2LAN) && !(ctl_subs->flag & SFE_CM_CTL_DIR_L2W))break;
				switch(ctl_subs->type)
				{
					case SFE_CM_RULE_SPORT:
						temp_port = ntohs(head->src_port);
						if (temp_port > ctl_subs->rule.port.max_port || temp_port < ctl_subs->rule.port.min_port)
							need_break = 1;
						break;
					case SFE_CM_RULE_DPORT:
						temp_port = ntohs(head->dest_port);
						if (temp_port > ctl_subs->rule.port.max_port || temp_port < ctl_subs->rule.port.min_port)
							need_break = 1;
						break;
					case SFE_CM_RULE_SPORT_XLATE:
						temp_port = ntohs(head->src_port_xlate);
						if (temp_port > ctl_subs->rule.port.max_port || temp_port < ctl_subs->rule.port.min_port)
							need_break = 1;
						break;
					case SFE_CM_RULE_DPORT_XLATE:
						temp_port = ntohs(head->dest_port_xlate);
						if (temp_port > ctl_subs->rule.port.max_port || temp_port < ctl_subs->rule.port.min_port)
							need_break = 1;
						break;
					case SFE_CM_RULE_SMAC:
					case SFE_CM_RULE_DMAC:
						if (memcmp(ctl_subs->rule.mac, head->reply_match->xmit_dest_mac,    ETH_ALEN) &&
						    memcmp(ctl_subs->rule.mac, head->reply_match->xmit_src_mac,     ETH_ALEN) &&
						    memcmp(ctl_subs->rule.mac, head->original_match->xmit_dest_mac, ETH_ALEN) &&
						    memcmp(ctl_subs->rule.mac, head->original_match->xmit_src_mac,  ETH_ALEN))
						    need_break = 1;
						break;
					default:
						need_break = 1;
						printk("sfe_ipv4 error: not support the type %d!\n", ctl_subs->type);
						break;
				}
				ctl_flag =  ctl_subs->flag & SFE_CM_SAME_BITS;
				if (need_break)
					break;
			}
			if (!ctl_subs)    //satisfy all subrules,so this rule matches this connection
			{
				ctl_conn = 1;
				break;
			}
		...
		if (ctl_conn)
		{
			/* if mark is different ,just give a warning and then set the mark*/
			if ((head->mark & MASK_FOR_IQOSMARK) && ( (head->mark & MASK_FOR_IQOSMARK) != (ctl_flag & MASK_FOR_IQOSMARK)))
			{
				printk("sfe_ipv4 warning: marks are different!\n");
			}
			head->mark &= (~MASK_FOR_IQOSMARK);
			head->mark |= (ctl_flag & MASK_FOR_IQOSMARK);
			head->set_qos_mark = ctl_flag & SFE_CM_CTL_MARK_ALL;
		}
	}
	spin_unlock_bh(&si->lock);
}
...
/*
 * sfe_ipv4_get_conn_max()
 */
static ssize_t sfe_ipv4_get_conn_max(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct sfe_ipv4 *si = &__si;
...
/*
 * sfe_ipv4_set_conn_max()
 */
static ssize_t sfe_ipv4_set_conn_max(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t size)
{
	struct sfe_ipv4 *si = &__si;
	strict_strtol(buf, 0, (long int *)&si->max_connections);

	return size;
}

static const struct device_attribute sfe_ipv4_conn_num_attr =
	__ATTR(max_connections, S_IWUSR | S_IRUGO, sfe_ipv4_get_conn_max, sfe_ipv4_set_conn_max);

/* Added by zjj, 20170117, for traffic statistics, use proc file from dbs modified. start--> */
#define TFS_ADDR_LEN (18)
#define TFS_MAX_CLIENT_NUM (64)
#define TFS_PROMPT_FORMAT "SFE STATISTICS:\n"
#define TFS_PRINT_FORMAT "ip=%s, mac=%s, tx_pkts=%llu, rx_pkts=%llu, tx_bytes=%llu, rx_bytes=%llu\n"

struct sfe_client_entry {
	uint64_t tx_packet;
	uint64_t rx_packet;
	uint64_t tx_byte;
	uint64_t rx_byte;
...
static struct sfe_client_entry l_tcb[TFS_MAX_CLIENT_NUM];
static DEFINE_SPINLOCK(tfs_client_lock);
static unsigned int tfs_client_number = 0;

unsigned int sfe_get_tfs_client_number()
{
	return tfs_client_number;
}

unsigned int sfe_get_ipv4_si_num()
{
	return __si.num_connections;
}

void tfs_update_client_sync_cb(struct sfe_connection_sync *sis)
{
	int i = 0;
	int flagMerged = 0;
	
	if (unlikely(!sis || sis->is_v6))
		return;
	
	for (i = 0; i < TFS_MAX_CLIENT_NUM; i++) {
		if ((0 == l_tcb[i].client_mac[0]) && (0 == l_tcb[i].client_mac[1])
		 && (0 == l_tcb[i].client_mac[2]) && (0 == l_tcb[i].client_mac[3])
		 && (0 == l_tcb[i].client_mac[4]) && (0 == l_tcb[i].client_mac[5]))
		{
			break;
		}
		
		if (0 == memcmp(sis->rep_dst_mac, l_tcb[i].client_mac, ETH_ALEN)) {
			flagMerged = 1;			
			spin_lock_bh(&tfs_client_lock);
			l_tcb[i].tx_packet += sis->src_new_packet_count;
			l_tcb[i].rx_packet += sis->dest_new_packet_count;
			l_tcb[i].tx_byte += sis->src_new_byte_count;
			l_tcb[i].rx_byte += sis->dest_new_byte_count;
			l_tcb[i].client_ip = sis->src_ip.ip;
			spin_unlock_bh(&tfs_client_lock);
			break;
		}
	}

	if (i >= TFS_MAX_CLIENT_NUM) {
		DEBUG_ERROR("%s: Too many clients for traffic statistics!!!.\n", __FUNCTION__);
		spin_lock_bh(&tfs_client_lock);
		memset(&l_tcb, 0, sizeof(l_tcb));
		spin_unlock_bh(&tfs_client_lock);
		return;
	}
	...
	if (!flagMerged)
	{
		spin_lock_bh(&tfs_client_lock);
		l_tcb[i].client_ip = sis->src_ip.ip;
		memcpy(l_tcb[i].client_mac, sis->rep_dst_mac, ETH_ALEN);
		l_tcb[i].tx_packet = sis->src_new_packet_count;
		l_tcb[i].rx_packet = sis->dest_new_packet_count;
		l_tcb[i].tx_byte = sis->src_new_byte_count;
		l_tcb[i].rx_byte = sis->dest_new_byte_count;
...
static int sfe_conn_proc_show(struct seq_file *m, void *v)
{
	int i  = 0;
	char rep_dst_mac[TFS_ADDR_LEN] = {0};
	char rep_dst_ip[TFS_ADDR_LEN] = {0};

	struct sfe_ipv4 *si = (struct sfe_ipv4 *)&__si;
	struct sfe_ipv4_connection *c;
	struct sfe_ipv4_connection_match *original_cm;
	struct sfe_ipv4_connection_match *reply_cm;
	int bytes_read;
...
static int sfe_conn_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, sfe_conn_proc_show, NULL);
}

static const struct file_operations sfe_conn_fops = {
	.open = sfe_conn_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static int sfe_stat_proc_show(struct seq_file *m, void *v)
{
	int i  = 0;
	char rep_dst_mac[TFS_ADDR_LEN] = {0};
	char rep_dst_ip[TFS_ADDR_LEN] = {0}; 

	seq_printf(m,"SFE CONNECTIONS: %u\n",__si.num_connections);
	seq_printf(m, TFS_PROMPT_FORMAT);
	...
	for (i = 0; i < TFS_MAX_CLIENT_NUM; i++)
	{
		...
		spin_lock_bh(&tfs_client_lock);		
		sprintf(rep_dst_ip, "%d.%d.%d.%d",
				(l_tcb[i].client_ip & 0xff), (l_tcb[i].client_ip << 16) >> 24,
				(l_tcb[i].client_ip << 8)  >> 24, l_tcb[i].client_ip >> 24);
		
		sprintf(rep_dst_mac, "%02x-%02x-%02x-%02x-%02x-%02x", 
					l_tcb[i].client_mac[0], l_tcb[i].client_mac[1], l_tcb[i].client_mac[2], 
					l_tcb[i].client_mac[3], l_tcb[i].client_mac[4], l_tcb[i].client_mac[5]);
		
		seq_printf(m, TFS_PRINT_FORMAT, rep_dst_ip, rep_dst_mac,
					l_tcb[i].tx_packet, l_tcb[i].rx_packet,
					l_tcb[i].tx_byte, l_tcb[i].rx_byte);
		spin_unlock_bh(&tfs_client_lock);
	}
...
static int sfe_stat_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, sfe_stat_proc_show, NULL);
}

static const struct file_operations sfe_stat_fops = {
	.open = sfe_stat_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
static int sfe_ipvr_proc_entry_create(void)
{
	if (!(sfe_ipv4_entry = proc_mkdir("sfe_ipv4", NULL))){
		DEBUG_ERROR("failed to register sfe_ipv4 proc entry\n");
		return -ENOENT;
	}

	if (!(sfe_stat_entry = proc_create("statistics",
                    S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH, sfe_ipv4_entry, &sfe_stat_fops))){
		if (sfe_ipv4_entry){
			remove_proc_entry("sfe_ipv4", NULL);
			sfe_ipv4_entry = NULL;
		}
		DEBUG_ERROR("failed to register sfe_ipv4 statistics proc entry\n");
		return -ENOENT;
	}

	if (!(sfe_conn_entry = proc_create("connections",
					S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH, sfe_ipv4_entry, &sfe_conn_fops))){
		if (sfe_ipv4_entry){
			remove_proc_entry("sfe_ipv4", NULL);
			sfe_ipv4_entry = NULL;
		}
		DEBUG_ERROR("failed to register sfe_ipv4 connections proc entry\n");
		return -ENOENT;
	}

    return 0;
}
/* <--Ended by zjj, 20170117, for traffic statistics, use proc file from dbs modified. */
...
static int __init sfe_ipv4_init(void)
{
	...
	result = sysfs_create_file(si->sys_sfe_ipv4, &sfe_ipv4_conn_num_attr.attr);
	if (result) {
		DEBUG_ERROR("failed to register sfe_ipv4_conn_num file: %d\n", result);
		goto exit4;
	}

	if (sfe_ipvr_proc_entry_create()) {
        DEBUG_ERROR("%s: Create sfe ipv4 proc entry failed\n", __func__);
        goto exit4;
    }
...
exit5:
	if (sfe_stat_entry){
		if (sfe_ipv4_entry){
			remove_proc_entry("statistics", sfe_ipv4_entry);
		}
		sfe_stat_entry = NULL;
	}
	if (sfe_conn_entry){
		if (sfe_ipv4_entry){
			remove_proc_entry("connections", sfe_ipv4_entry);
		}
		sfe_conn_entry = NULL;
	}
	if (sfe_ipv4_entry) {
...
exit3:
#endif /* CONFIG_NF_FLOW_COOKIE */
	sysfs_remove_file(si->sys_sfe_ipv4, &sfe_ipv4_conn_num_attr.attr);
#ifdef CONFIG_SFE_DEBUG_DEV
...
EXPORT_SYMBOL(sfe_ipv4_ctl_conns);
EXPORT_SYMBOL(sfe_get_tfs_client_number);
EXPORT_SYMBOL(sfe_get_ipv4_si_num);
EXPORT_SYMBOL(tfs_update_client_sync_cb);
