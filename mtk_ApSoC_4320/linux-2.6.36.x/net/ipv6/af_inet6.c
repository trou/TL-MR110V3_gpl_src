...
__u32 inet6_getifflags(struct net *net, void __user *arg)
{
	struct inet6_dev *idev;
	struct net_device *dev;
	char ifname[IFNAMSIZ];

	if (copy_from_user(ifname, arg, sizeof(ifname)))
		return -EFAULT;
...
int inet6_ioctl(struct socket *sock, unsigned int cmd, unsigned long arg)
{
	...
	case SIOCGIP6IFFLAGS:
		return inet6_getifflags(net, (void __user *) arg);
	default:
		if (!sk->sk_prot->ioctl)
			return -ENOIOCTLCMD;
