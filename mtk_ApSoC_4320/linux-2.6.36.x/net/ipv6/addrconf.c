...
void addrconf_prefix_rcv(struct net_device *dev, u8 *opt, int len)
{
	...
	if (pinfo->onlink) {
		...
		if (rt && addrconf_is_prefix_route(rt)) {
			...
			/*added by xieping, lte ipv6 no need to add prefix route*/
			...
			if(strcmp(dev->name, "wwan0") != 0)
#elif defined(CONFIG_TP_MODEL_MR100V1) ||  defined(CONFIG_TP_MODEL_MR100V2) ||  defined(CONFIG_TP_MODEL_MR402V1)
			if(strcmp(dev->name, "lte0") != 0)
#endif
...
#ifdef CONFIG_IPV6_ROUTER_WAN_SLAAC	/* Add by HYY, 16Apr13 */
			char tmpSlaacAddr[256] = {0};
			sprintf(tmpSlaacAddr, "%pI6 %d", &addr, pinfo->prefix_len);
			if(strcmp(tmpSlaacAddr,in6_dev->cnf.slaac_addr))
				sprintf(in6_dev->cnf.slaac_addr, "%pI6 %d", &addr, pinfo->prefix_len);
#endif
...
static void addrconf_dad_completed(struct inet6_ifaddr *ifp)
{
	...
	if ((ifp->idev->cnf.forwarding == 0 &&
		...
		(ifp->idev->dev->priv_flags & IFF_WAN_DEV) &&
		(ipv6_addr_type(&ifp->addr) & IPV6_ADDR_LINKLOCAL)))
{
		/*
