		  ...
		  struct packet_type *ptype, struct net_device *orig_dev)
{
	...
	struct net_device *vlan_dev_sec;
	u16 vlan_id;
	u16 vlan_tci;
	...
	{
		vlan_id = 0 & VLAN_VID_MASK;

		vlan_dev_sec = __find_vlan_dev(vlan_dev, vlan_id);
		if(vlan_dev_sec)
		{
			skb->dev = vlan_dev_sec;
					    ...
					    struct net_device *dev)
{
	    ...
	    vlan_dev_info(dev)->flags & VLAN_FLAG_REORDER_HDR) */
	{
		...
		{
			pr_debug("\n vlanID is %d, do not put tag",vlan_tci);
		}
	}
