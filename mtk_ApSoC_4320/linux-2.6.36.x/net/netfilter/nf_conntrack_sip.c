 ...
 *
 * 20130625 modified by tp-link hanjiayan.
 */
...
static void dump_sip_info(void)
{
	...
	{	
		...
		printk("\t%d. expire %lu, exp dst %u.%u.%u.%u:%u, local %u.%u.%u.%u:%u\n",
				i,
				(j->timeout.expires - jiffies)/HZ,
				NIPQUAD(j->exp_dst_ip), j->exp_dst_port,
				NIPQUAD(j->local_ip), j->local_port);				
		i++;
	}
out:	
...
int addSipInfo(__be32 dst_ip, __be16 dst_port,
...
//	typeof(nf_nat_sdp_port_hook) nf_nat_sdp_port;
	...
	if(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip == 
		...
		ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip == daddr->ip)
	{
		SIP_ALG_DBG("No need to nat media, don't build RTP expectation!");
		return NF_ACCEPT;
	}
	...
	if (ct->tuplehash[dir].tuple.src.u3.ip 
 		== ct->tuplehash[!dir].tuple.dst.u3.ip)
	{
		SIP_ALG_DBG("Remote in SDP, don't need RTP expectation!");
		return NF_ACCEPT;
	}
	else
	{
		SIP_ALG_DBG("Local out SDP, Build RTP expectation!");
	}
	...
	if (nf_nat_sip_expect && ct->status & IPS_NAT_MASK)
	{
		...
		if(NF_ACCEPT == ret && exp)
		{
			addSipInfo(exp->tuple.dst.u3.ip, exp->tuple.dst.u.all, 
				exp->saved_ip, exp->saved_proto.all, SIP_EXPECT_SIGNALLING_TIMEOUT);
		}
	}
	else {
		if (nf_ct_expect_related(exp) != 0)
...
#if (0)
/* #349094. Deny driver 'rmmod' facility for it does not work properly, anyway */
