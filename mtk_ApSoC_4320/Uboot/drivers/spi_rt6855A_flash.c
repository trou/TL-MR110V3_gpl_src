...
int do_mem_cp(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	/* Port from soho, yuanshang, 2013-9-16 */
	/* added by HouXB, 04Nov11 */	
	else if (!strncmp(argv[0], "cp.b", 5))
	{	
		addr = simple_strtoul(argv[1], NULL, 16);		
		dest = simple_strtoul(argv[2], NULL, 16);		
		count = simple_strtoul(argv[3], NULL, 16);				
		printf("\n Copy 0x%08X to 0x%08X, count 0x%X.... \n",	addr, dest, count); 
	}	
	else {
		printf ("Usage:\n%s\n", cmdtp->usage);
		return 1;
...
int do_flerase (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	/* add erase flash by user define, yuanshang, 2013-9-16 */
	else if (strcmp(argv[1], "tplink") == 0)
	{
		unsigned int addr = 0;
