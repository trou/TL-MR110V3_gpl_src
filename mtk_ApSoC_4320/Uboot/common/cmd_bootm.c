...
int do_bootm (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	char	*name;
	image_header_t *hdr = &header;
	unsigned char	*fileTag; 		// by lqm, 17Sep07s
	...
	printf("do_bootm:argc=%d, addr=0x%x\n", argc, addr);
	...
	fake_image_header(hdr, *((unsigned long *)(fileTag+84)), *((unsigned long *)(fileTag+88)), *((int *)(fileTag+ 96)));
...
/*	case IH_OS_LINUX: */
	...
	**	TP GPIO features
	**
	**	Now set LEDs of all color off (for wa855 requriment)
	@@	By TengFei, 13/07/23
	*/
#ifdef	PRODUCT_GPIO_FEATURE
	gpio_led_set(RALINK_LED_RED_GPIO_NUM, 0);
	gpio_led_set(RALINK_LED_YELLOW_GPIO_NUM, 0);
	gpio_led_set(RALINK_LED_GREEN_GPIO_NUM, 0);
