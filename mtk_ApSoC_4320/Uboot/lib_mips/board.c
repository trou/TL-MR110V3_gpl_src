...
#define RALINK_GPIOMODE_REG_1       (RALINK_SYSCTL_BASE+0x60)
#define RALINK_GPIO_CTRL_32_63      (RALINK_PIO_BASE+0x4)
#define RALINK_GPIO_DATA_32_63      (RALINK_PIO_BASE+0x24)
/************************************************************************
 *
...
/* The codes below get 0x35 and 0x36 in eeprom partition to check if the model enables ePA and/or eLNA.
	But the partition type of Uboot is not the same as the model. So DO NOT USE codes below and use macro instead.*/
#if 0
		char ee35, ee36;
...
#endif
#if defined(EXTERNAL_RF_SWITCH)
/* only models with external PA and/or external LNA need to set it. */
		{
			RALINK_REG(RALINK_SYSCTL_BASE+0x60)|= ((0x3<<24));
		}
#endif
#if defined(EXTERNAL_PA_LNA)
/* only models with external PA and/or external LNA need to set it. */
		{
			RALINK_REG(RALINK_SYSCTL_BASE+0x60)|= ((0x3 << 6));
		}
#endif
	}
	...
	//for MT7628
	//Set PERST_GPIO_MODE GPIO#36 to GPIO mode
	old_value = le32_to_cpu(*(volatile u_long *)RALINK_GPIOMODE_REG_1);
	new_value =  old_value | (0x1<<16);
	*(volatile u_long *)(RALINK_GPIOMODE_REG_1) = cpu_to_le32(new_value);
	printf("RALINK_GPIOMODE_REG_1(0x%x) old=0x%x new=0x%x\n", RALINK_GPIOMODE_REG_1, old_value, new_value);

	//Set GPIO_CTRL_1 GPIO#36 输出
	old_value = le32_to_cpu(*(volatile u_long *)RALINK_GPIO_CTRL_32_63);
	new_value = old_value | (0x1 << 4);
	*(volatile u_long *)(RALINK_GPIO_CTRL_32_63) = cpu_to_le32(new_value);
	printf("RALINK_GPIO_CTRL_32_63(0x%x) old=0x%x new=0x%x\n", RALINK_GPIO_CTRL_32_63, old_value, new_value);

	//Set GPIO_DATA_1 GPIO#36输出低电平
	old_value = le32_to_cpu(*(volatile u_long *)RALINK_GPIO_DATA_32_63);
	new_value = old_value & (~(0x1 << 4));
	*(volatile u_long *)(RALINK_GPIO_DATA_32_63) = cpu_to_le32(new_value);
	printf("RALINK_GPIO_DATA_32_63(0x%x) old=0x%x new=0x%x\n", RALINK_GPIO_DATA_32_63, old_value, new_value);
...
*/
	printf(" estimate memory size =%d Mbytes\n",gd->ram_size /1024/1024 );
...
/*config bootdelay via environment parameter: bootdelay */
	...
	/* For debug default enter command line mode, yuanshang, 2013-09-13 */
	BootType = '3';
	/* Only need two mode, 1.default mode:boot system via flash; 2.develop mode:press 't' into command line interface  */
	/*OperationSelect();*/
	while (timer1 > 0) {
		--timer1;
		/* delay 100 * 10ms */
		...
		if (BootType != '4'/* goto cmd */ && BootType != '7' /* serial upgrade boot.bin */)
		{
			...
			if (icount == 0)
			{
				...
				run_command("set serverip 192.168.0.225", 0);
#if defined(TP_MODEL_MR100V1)
				run_command("tftp 0x80060000 tp_recovery.bin;erase tplink 0x10000 0x3d0000;cp.b 0x80080000 0x10000 0x3d0000", 0);
#else
	...
	if(BootType == '3') {
...
#endif
		{
			//udelay (10000 * 100 * 2);
			//rt_mac_status();
			//rt_rtl8367_phy_status();
			printf("switch BootType:\n");
		}

		switch(BootType) {
/* no need, yuanshang, 2013-9-16 */
#if 0			
		case '1':
...
MIN_FAILED:
	...
	if (cid==1)
    {
        /* For Zentel DDR2(2160500619) crash issue, disable Self-Refresh feature. */
        RALINK_REG(RALINK_MEMCTRL_BASE+0x10) &= ~(0x1<<4);
    }
#elif defined (MT7620_FPGA_BOARD) || defined (MT7620_ASIC_BOARD)
	/* For Zentel DDR2(2160500619) and ESMT crash issue, disable Self-Refresh feature. */
	RALINK_REG(RALINK_MEMCTRL_BASE+0x18) &= ~(0x1<<4);
#else
	RALINK_REG(RALINK_MEMCTRL_BASE+0x18) |= (0x1<<4);
#endif
