...
int start_pppd (struct call *c, struct ppp_opts *opts)
{
	...
	stropt[pos++] = strdup("username_len");
	stropt[pos++] = strdup(gconfig.username_len);
	stropt[pos++] = strdup("password_len");
	stropt[pos++] = strdup(gconfig.password_len);
#else
	stropt[pos++] = strdup("user");
	stropt[pos++] = strdup(gconfig.user);
	stropt[pos++] = strdup("password");
	stropt[pos++] = strdup(gconfig.password);
#endif /* USE_FILE_TO_PASS_PPP_ARG */
...
void init_args(int argc, char *argv[])
{
...
#ifdef USE_FILE_TO_PASS_PPP_ARG
    memset(gconfig.username_len, 0, sizeof(gconfig.username_len));
    memset(gconfig.password_len, 0, sizeof(gconfig.password_len));
#else
    memset(gconfig.user, 0, SecretsLen);
    memset(gconfig.password, 0, SecretsLen);
#endif /* USE_FILE_TO_PASS_PPP_ARG */
    ...
    for (i = 1; i < argc; i++) {
        ...
        /* Add by chz for new parameters, 2013-03-14 */
#ifdef USE_FILE_TO_PASS_PPP_ARG
		else if (! strncmp(argv[i], "username_len", 12)) {
			if (++i == argc)
				usage();
			else
				strncpy(gconfig.username_len, argv[i], sizeof(gconfig.username_len) - 1);
		}
		else if (! strncmp(argv[i], "password_len", 12)) {
			if (++i == argc)
				usage();
			else
				strncpy(gconfig.password_len, argv[i], sizeof(gconfig.password_len) - 1);
		}
#else
...
route_add(const struct in_addr inetaddr, struct rtentry *rt)
{
	...
	while (fgets(buf, sizeof(buf), f)) 
	{
		...
		if ((flags & RTF_UP) == RTF_UP && (inetaddr.s_addr & mask) == dest)
		{
			if (dest == 0)
			{
				def = sin_addr(&rt->rt_gateway).s_addr;
				def_metric = metric;
				def_mask = mask;
				sprintf(def_dev,"%s",dev);
				continue;
			}
	...
	if (rt->rt_gateway.sa_family != AF_INET) 
	{
		if (def!=0)
		{
			sprintf(dev,"%s",def_dev);
			mask = def_mask;
			sin_addr(&rt->rt_gateway).s_addr = def;
			rt->rt_metric = def_metric + 1;
			rt->rt_gateway.sa_family = AF_INET;
		}
		else
		{
