...
static int get_sub_alt_name_type(char **san)
{
    int type = 0;
    char *p = *san;
    ...
    if ((strlen(p) >= 4) && !strncmp(p, "DNS:", 4)) {
        p += 4;
        type = MBEDTLS_X509_SAN_DNS_NAME;
    } else if ((strlen(p) >= 5) && !strncmp(p, "IP:", 3)) {
        p += 3;
...
static void get_sub_alt_name_content(char **san)
{
    char *p = *san;
...
int mbedtls_x509write_crt_set_subject_alt_name( mbedtls_x509write_cert *ctx,
                                         		const unsigned char *san, size_t size )
{
    int ret;
	...
	/* Now support DNS and IPv4 address. The 'san' format is like "DNS:xxx.net,IP:192.168.1.1" */
	while (*p != '\0')
	{
		type = get_sub_alt_name_type(&p);
		index = p;
		get_sub_alt_name_content(&p);
		if (index == p) break;
		
		if (type == MBEDTLS_X509_SAN_DNS_NAME)
		{
			MBEDTLS_ASN1_CHK_ADD( len,
								  mbedtls_asn1_write_tagged_string( &c, buf,
														  MBEDTLS_ASN1_CONTEXT_SPECIFIC |
														  MBEDTLS_X509_SAN_DNS_NAME,
														  index, p - index ) );
		}
		else if (type == MBEDTLS_X509_SAN_IP_ADDRESS)
		{
			memcpy(ipstr, index, p - index);
			if (!ipv4_from_asc(ipaddr, ipstr)) continue;
			MBEDTLS_ASN1_CHK_ADD( len,
								  mbedtls_asn1_write_tagged_string( &c, buf,
														  MBEDTLS_ASN1_CONTEXT_SPECIFIC |
    ...
    return mbedtls_x509write_crt_set_extension( ctx, MBEDTLS_OID_SUBJECT_ALT_NAME,
